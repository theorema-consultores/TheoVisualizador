/*
 * Experimento de desempenho do TheoView.
 *
 * Mantém a regra da fonte paga e troca as 12 consultas mensais por uma
 * consulta anual por entidade. Compare linhas e totais com a fonte oficial.
 */
...
try {
  ...
  final def VERSAO_GERADOR = "2026-09-14-04-otimizado-teste"
  ...
  def buscarMovimentosAno = { ano, entidade ->
    ...
    try {
      ...
      imprimir "[GERADOR] Fonte por exercício falhou para entidade " + entidade +
        "; usando fonte mensal. Motivo: " + erroFonteExercicio.toString()
  ...
  def resultados = exercicios.collect { ano ->
    ...
    [
      ...
      relatorio: [
        nome: "Balancete da despesa - teste otimizado",
        exercicio: ano,
  ...
  def arquivoResultado = Arquivo.novo("balancete-despesa-otimizado-teste.json", "json")
  arquivoResultado.escreverObjeto(dadosJson)

  variaveis.visao.adicionarRelatorio(
    id: "balancete-despesa-otimizado-teste",
    version: "1.0.0",
    nome: "balancete-despesa-otimizado-teste.json",
