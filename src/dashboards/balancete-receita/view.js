...
export function renderBalancete(container, model) {
  ...
  const monthDropdown = element(doc, 'div', '', 'month-dropdown dropdown');
  const monthSelect = doc.createElement('button');
  monthSelect.type = 'button';
  monthSelect.className = 'month-select dropdown-toggle';
  ...
  const monthList = element(doc, 'div', '', 'month-options dropdown-menu');
  monthList.setAttribute('role', 'listbox'); monthList.hidden = true;
  MONTHS.forEach((month, index) => {
    const option = element(doc, 'button', month, 'month-option dropdown-item');
    option.type = 'button'; option.dataset.value = String(index); option.setAttribute('role', 'option');
    option.addEventListener('click', () => { monthLabel.textContent = month; monthSelect.dataset.value = String(index); monthList.hidden = true; monthSelect.setAttribute('aria-expanded', 'false'); updateMonth(); });
    monthList.append(option);
  });
  const initialMonth = new Date().getMonth();
  const monthLabel = element(doc, 'span', MONTHS[initialMonth], 'month-label');
  const monthIcon = element(doc, 'span', 'expand_more', 'material-symbols-rounded month-icon');
  monthSelect.append(monthLabel, monthIcon); monthSelect.dataset.value = String(initialMonth);
